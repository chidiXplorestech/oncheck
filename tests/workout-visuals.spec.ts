...
test('Training artwork follows the existing Monday Wednesday Thursday structure', async ({ page }) => {
  ...
  await expect(banner.locator('img')).toHaveAttribute('src', /workouts\/monday-trunk\.png$/);
  ...
  await expect(banner.locator('img')).toHaveAttribute('src', /workouts\/wednesday-lower-body\.png$/);
  ...
  await expect(banner.locator('img')).toHaveAttribute('src', /workouts\/thursday-arms\.png$/);
